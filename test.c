...
struct semtest_type g_semtest;
xSemaphoreHandle    g_mutex = NULL;

static const uint8_t crc8_h2f_xor       = 0xff;
static const uint8_t crc_8_h2f_tbl[256] = {
...
uint8_t crc8_h2f(uint8_t *buf, uint8_t len)
{
    uint8_t  index;
    uint8_t  crctemp;
    uint8_t  result;
...
{
    memset(test->buf, TEST_SEMBUF_INITVAL, TEST_SEMBUF_SIZE);
    test->crc = crc8_h2f(test->buf, TEST_SEMBUF_SIZE);
}

int test_semaphore(struct semtest_type *test, uint8_t *crc_cal)
{
    uint8_t value;
    uint8_t array [TEST_SEMBUF_SIZE];
    int index;
    int ret = 0;

    *crc_cal = crc8_h2f(test->buf, TEST_SEMBUF_SIZE);
    ...
    for (index = 0; index < TEST_SEMBUF_SIZE; index++)
    {
        value = (uint8_t)(STM0_TIM0.U & 0xFF);
        array[index] = value;
    }

    test->crc = crc8_h2f(array, TEST_SEMBUF_SIZE);
    memcpy(test->buf, array, TEST_SEMBUF_SIZE);
